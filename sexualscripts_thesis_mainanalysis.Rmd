...
packages <- c('stats', 'readr', 'dplyr', 'ggplot2', 'psych', 'lavaan', 'semPlot', 'semTable', 'IsingFit', 'qgraph', 'igraph', 'IsingSampler', 'psychonetrics', 'EGAnet', 'EFAtools', 'missMethods', 'norm')
...
figure_seabrook_cfa_higherfactor_fit <- semPaths(
  ...
  curvePivot = TRUE, # making curved arrows
  esize = 1, # thickness of arrows
  nCharNodes = 10, # uses 8 characters of the name in the boxes
  edge.label.cex = 0.75,
  node.label.cex = 8,
  residuals = FALSE, # does not show the error terms/residuals
  sizeMan = 4, # width of manifest var box
  sizeLat = 8,
  layout = 'tree2'
)
...
cormat_allscales <- cor(data_networkmodel)
...
graph_network_full <- 
  qgraph(getmatrix(networkmodel_final, "omega"),
         labels = networklabels,
...
walktrap_full <- 
  walktrap.community(as.igraph(graph_network_full),
                     weights = abs(E(as.igraph(graph_network_full))$weight))


graph_network_full_walk <- 
  qgraph(getmatrix(networkmodel_final, "omega"),
         labels = networklabels,
         layout = network_graph_reduced_1$layout,
         vsize = 4,
